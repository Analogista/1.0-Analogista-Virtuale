...
import { useApiKey } from '../contexts/ApiKeyContext'; // Import Context
...
const TestInduttore: React.FC<ScreenProps> = ({ setPage, onNext, isWizard }) => {
  ...
  const handleCameraReady = () => {
    if (videoRef.current) {
        // Non inizializziamo il detector interno di voiceService per risparmiare risorse.
        // voiceService ora ascolta gli eventi globali emessi dal detector di CameraView.
  ...
  const handleCalibrated = () => {
      // voiceService non ha bisogno di calibrazione propria perché usa gli eventi di CameraView
      playInstructions();
  };
  ...
  const playInstructions = async () => {
    ...
    try {
      ...
      // Only set error if not cancelled/repeated
  ...
  const startTest = async () => {
    ...
    try {
      ...
      await voiceService.speak("premi il pulsante qui sotto per proseguire con gli altri test oppure riprova questo test");
        ...
        if (status !== 'WAITING_INTRO') {
            setMessage("Test interrotto.");
        }
    }
  };

  // Ascolta per comandi vocali (Disattivato per automatizzare il flusso)
  useEffect(() => {
    /* 
    if (status === 'READY_TO_START') {
        voiceService.listenForCommand('Avvia', () => {
            startTest();
        });
    }
    */
  ...
  return (
    <div className={`${isProMode ? 'bg-[#0a0a0c] text-white' : 'bg-white text-gray-800'} min-h-screen p-4 sm:p-6 pb-24 relative transition-colors duration-500`}>
      <div className="flex justify-between items-start mb-6">
        <div>
          <h2 className={`text-2xl font-serif font-black ${isProMode ? 'text-cyan-400' : 'text-gray-800'}`}>
            {isWizard ? 'STEP 3: TEST INDUTTORE' : '6) TEST INDUTTORE'}
          </h2>
      ...
      {!isProMode && <VideoAccordion videoId="15XsRc0Z5w8" title="Spiegazione: Cosa sono Induttore Destro e Sinistro?" />}
      
      {!isProMode && (
        <div className="mb-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200 text-sm">
            <h4 className="font-bold text-gray-800 mb-1">Grillo e Lucignolo</h4>
            <p className="text-gray-700">Regole e Trasgressione.</p>
        </div>
      )}
      ...
      <div className="my-8 flex justify-center gap-4">
        {status === 'WAITING_CALIBRATION' && (
            <button
                ...
                className="w-full bg-cyan-600 text-white font-black py-4 px-8 rounded-2xl transition duration-300 disabled:opacity-30 uppercase tracking-widest shadow-xl"
            >
                Inizia Calibrazione
            </button>
        )}
        ...
        {status === 'WAITING_INTRO' && (
             <div className="flex items-center space-x-3 bg-cyan-950/20 px-6 py-4 rounded-2xl border border-cyan-500/30">
                <span className="text-cyan-400 font-black uppercase tracking-widest text-sm text-center w-full">Configurazione Audio...</span>
           </div>
        )}
       ...
       <div className={`text-center p-6 rounded-2xl min-h-[100px] flex flex-col justify-center border transition-all ${
         isProMode ? 'bg-gray-900/20 border-gray-800' : 'bg-gray-50 border-gray-100'
       }`}>
        <p className={`font-medium text-lg leading-relaxed ${
          status === 'SUCCESS' ? 'text-green-400' : (status === 'DONE' ? 'text-orange-400' : (isProMode ? 'text-gray-300' : 'text-blue-800'))
        }`}>
          {message}
        </p>
      ...
      {(status === 'SUCCESS' || status === 'DONE') && (
        <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
            <button 
                ...
                className="flex-1 bg-gray-800 text-gray-400 font-black py-4 px-6 rounded-2xl hover:bg-gray-700 transition duration-300 uppercase tracking-widest border border-white/5"
            >
                RIPROVA
            </button>
            <button 
                onClick={handleProsegui} 
                className="flex-1 bg-cyan-600 text-white font-black py-4 px-6 rounded-2xl hover:bg-cyan-500 transition duration-300 shadow-xl uppercase tracking-widest"
            >
                PROSEGUI →
            </button>
        </div>
